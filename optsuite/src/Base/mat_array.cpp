...
namespace OptSuite { namespace Base {
    ...
    template<typename dtype>
    void MatArray_t<dtype>::assign(const Variable<dtype>& other){
    ...
    template<typename dtype>
    Scalar MatArray_t<dtype>::squared_norm() const {
    ...
    template<typename dtype>
    std::string MatArray_t<dtype>::to_string() const {
        std::stringstream out;
        for (auto&& i = this->begin(); i != this->end(); ++i){
            Index ind = std::distance(this->begin(), i);
            out << "[" << ind <<"]" << std::endl;
            out << *i << std::endl;

            if (ind != total_blocks() - 1)
                out << std::endl;
        }
        return out.str();
    }
