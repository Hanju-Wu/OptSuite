...
int main(int argc, char **argv){
    Mat A;
    SpMat spA;
    get_matrix_from_file("~/tmp/example.mat", "A", A);
    get_matrix_from_file("~/tmp/example.mat", "spA", spA);

    // construct factorized mat and sparse
    FactorizedMat<Scalar> mf(A.transpose(), A.transpose());
    SpMatWrapper<Scalar> ms(spA);

    // construct op
    FactorizePSpMatOp<Scalar> Aop(mf, 0.1_s, ms);

    auto t = tic();
    LANSVD<Scalar> lansvd;
    lansvd.options().set_from_cmd_line(argc, argv);
    lansvd.compute(Aop, 20);
