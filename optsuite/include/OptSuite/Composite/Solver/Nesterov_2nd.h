/*
 * @Author: Wu Hanju 
 * @Date: 2022-09-11 23:04:03 
 * @Last Modified by: Wu Hanju
 * @Last Modified time: 2022-09-12 16:32:07
 */
#ifndef OPTSUITE_ALGO_Nesterov_2nd
#define OPTSUITE_ALGO_Nesterov_2nd
...
namespace OptSuite { namespace Composite { namespace Solver {
    ...
    class Nesterov_2nd : public SolverBase {
        public:
            ...
            inline Nesterov_2nd() : SolverBase("Nesterov_2nd") {
                logger = std::make_shared<Logger>();
                register_options();
                set_variable_type<>();
                // set_default_options();
            }
            ~Nesterov_2nd() = default;
            ...
            void set_variable_type(){
                ...
                z = std::shared_ptr<xtype>(new xtype{});

                gx = std::shared_ptr<gtype>(new gtype{});
                gxp = std::shared_ptr<gtype>(new gtype{});
                gz = std::shared_ptr<gtype>(new gtype{});
            }
            ...
            void solve(const Model::Base<dtype>&, const Ref<const mat_t>, const Scalar tau0 = 1e-3);
            void solve(const Model::Base<dtype>&, const var_t&, const Scalar tau0 = 1e-3);
            OptionList& options();
        ...
        private:
            // local storage
            // mat_array_t x, gx, gz, y, z;
            var_t_ptr x, xp, gx, gxp, gz, y, yp, z;
            ...
            void set_z(var_t_ptr, var_t_ptr, var_t_ptr, Scalar);
            void set_x(var_t_ptr, var_t_ptr, var_t_ptr, Scalar);
